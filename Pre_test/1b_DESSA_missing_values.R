...
# ---- 1B.1. Examine prevalence of missing values ----

# ~ Subset the data ----

sel_data_items <- dplyr::select(sel_data, Date_interview_start:Q72)
names(sel_data_items)

# ~ Determine percent missing for each item ----

cases <- nrow(sel_data_items)
cases
perc_missing <- sapply(sel_data_items, function(x) sum(is.na(x))/cases)
View(perc_missing)

# ~ Table of missing value patterns ----
# Missing data pattern : md.pattern() - is from the "mice" package.
table_missing <- md.pattern(sel_data_items)
View(table_missing)
...
# ---- 1B.2. Visualize patterns of missing values ----

# ~ Using the mice package ----
mice_plot <- aggr(sel_data_items, col = c('navyblue','yellow'),
                  numbers = TRUE, sortVars = TRUE,
                  labels = names(sel_data_items), 
                  cex.axis = .7, cex.numbers = 0.3,
                  gap = 3, ylab = c("Missing Data", "Pattern"))

# ~ Using the naniar package ----
naniar::gg_miss_var(sel_data_items)   # This graph is elegant and intuitive


# ---- 1B.3. Impute missing data ----
# Results in new data set: sel_data_imputed

# IMPORTANT :
# Let this peace of code run, it will take some time (30 minutes ~ 1h+ ?!) ... ... do not rush it!!!
# The missing values method computed by the MICE package to deal with missing data has given an error due (I suppose)
# to the method used to treat the missing values and the high possible correlation between the variables (Q1, Q2, Q3, ... or Qn).
...
temp_sel_data <- mice(sel_data_items, m = 5, maxit = 50, method = 'sample', seed = 500)
